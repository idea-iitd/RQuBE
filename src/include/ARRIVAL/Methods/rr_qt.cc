...
int RandomWalkQT(int src, int dst, Graph *g, automata *nodeAutomata, Function fns, Random *rand, int max_pen = 2)
{
			...
			for (int lengthCounter = 0; lengthCounter < g->walkLength; ++lengthCounter)
			{
				...
				for (int k = 0; k < numChild; ++k)
				{
					...
					for (int labelChecker = 0; labelChecker < allNodeLabels.size(); ++labelChecker)
					{
						...
						vector<int> parseparams;
						parseparams.push_back( fns.params[nodeLabel]);
						parseparams.push_back(currNode->attributes[fns.funattr[nodeLabel]]);
						
						(*fns.functions[nodeLabel])(parseparams, &ans);
						if (fns.fnType[nodeLabel] != 0)
						{
							secans = 0;
							vector<int> parseparams2;
							
							parseparams2.push_back( fns.secparams[nodeLabel]);
							parseparams2.push_back(currNode->attributes[fns.secfunattr[nodeLabel]]);
							(*fns.secfunctions[nodeLabel])(parseparams2, &secans);
						}
						switch (fns.fnType[nodeLabel])
						{
							case 2:
							if (secans == 1)
							ans = 1;
						...
						if (ans == 1  && fwdCntr[node][nodeState] < max_pen)
						{
							flag = true;
							break;
						}
			...
			for (int lengthCounter = 0; lengthCounter < g->walkLength; ++lengthCounter)
			{
				...
				for (int k = 0; k < numChild; ++k)
				{
					...
					for (int labelChecker = 0; labelChecker < allNodeLabels.size(); ++labelChecker)
					{
						...
						if (ans == 1 && bwdCntr[node][nodeState] < max_pen)
						{
							flag = true;
							break;
						}
